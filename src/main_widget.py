...
class Dayswidget(Switcher_window):
    ...
    def forward(self, forward):
        ...
        difference_days = self.active_week_day[self._current_active_week] - \
            datetime.today().weekday()
        current_day = datetime.today()+timedelta(days=difference_days,
                                                 weeks=self._current_active_week)
    ...
    def getitem(self, addedday: date = date.today(), data: list = []):
        ...
        if widget.day >= date.today():
            Button(widget.interior, text="add task", command=widget.add_item).grid(
                row=widget.interior.grid_size()[1], column=0)
...
def main():
    ...
    Button(text="get keys", command=lambda: print(
        days_widget.get_keys())).pack()
    Button(text="Save", command=lambda: days_widget.save_file(
        "D:\Projects\Focusing\\save.pkl")).pack()
    root.mainloop()
