...
RIGHTARROW = Image.open(resource_path(os.path.join("public", "icons\\go_right.png")))
DOWNARROW = Image.open(resource_path(os.path.join("public", "icons\\down-arrow2.png")))
CATEGORIES = {0: None}
CONTAINER_FRAME_STYLE = dict(padx=14, pady=2)


class Title_ExplorerBar(Achievement_label):
    def __init__(
        self,
        app=None,
        defaultState=True,
...
class Description_title_explore(Title_ExplorerBar):
    def __init__(
        self,
        app=None,
        ...
        starting_state=False,
        description="",
        categoryId: datetime.datetime = 0,
        **kwargs,
    ):
    ...
    def get_item(self, parent: Widget):
        ...
        self.progress_bar = ttk.Progressbar(
            self.containerFrame, orient=HORIZONTAL, variable=self._progress_bar
        )
    ...
    def _trace_time(self):
        ...
        dead_line, current_time = (
            var
            if var != None
            else (
                self._float(self._time.get()),
                (
