...
STYLE_ENTRY_ACHIEVEMENT=dict(font=20,)
...
class Achievement_label(Frame):
    def __init__(self,app=NONE,default_text="",start_time:datetime.datetime=datetime.datetime.now(),last_paused_time:datetime.datetime=datetime.datetime.now(),time=0,every_time=20,end_target_state=False,failing=False,pausing_time=0,pausing_state=True,starting_state=False,**kwargs):
        """
        @parameters
    ...
    def get_item(self,parent:Widget):
        ...
        self.missionEntry=Entry(parent,text=self._missionVar.get(),textvariable=self._missionVar,**STYLE_ENTRY_ACHIEVEMENT)
        
        self.time_entry=Entry(parent,text=str(self._time.get()),textvariable=self._time,validatecommand =(reg, '%S'),width=10,**STYLE_ENTRY_ACHIEVEMENT)
        
        
        self.every_time_entry=Entry(parent,text=str(self._every_time.get()),textvariable=self._every_time,validatecommand =(reg, '%S'),width=10,**STYLE_ENTRY_ACHIEVEMENT)
            ...
            if not self.pausing_state and self.starting_state:
                ...
                if dead_line_time>0 and not self.failing:
                    ...
                    if current_time>=dead_line_time:
                        say(VOICE_ASSISTANT_SAYING["FINISHING_TIME"].format(self._missionVar.get()))
                        self.failing=True
                    ...
                    #check if the half time has been reached or not
                        ...
                        remaining_phase=VOICE_ASSISTANT_SAYING["REAMING_TIME_PASSED_TIME"].format(self._missionVar.get(),remaining_time_sting)
                        say(VOICE_ASSISTANT_SAYING["HALFTIME_PASSED"].format(self._missionVar.get(),remaining_phase))
                ...
                if self._float(self._every_time.get())>0 and \
                    passed_time>=self._float(self._every_time.get()) and not self.failing:
                    self._last_check=datetime.datetime.now()
                    say(VOICE_ASSISTANT_SAYING["MINUTE_PASSED"].format(self._time2string(passed_time)))
                    if dead_line_time>0:
                        string_time=self._time2string(remaining_time)
                        if string_time!="":  
                            say(VOICE_ASSISTANT_SAYING["REAMING_TIME_PASSED_TIME"].format(self._missionVar.get(),string_time))
